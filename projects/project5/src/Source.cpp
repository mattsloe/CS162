...
void loadData(SongType list[], int& size, const char infile[])
{
    ...
    while(!in.eof()){
        ...
        appendSong(list,size,currentSong); //add song to the list
        in.getline(inString,STR_MAX_SIZE-1,';'); //get song name
    }
...
{
    songIndex--; //convert to 0-based index
        cout << "Song " << songIndex+1 << ": " << songs[songIndex].songName << " by " << songs[songIndex].artist <<  " | " << songs[songIndex].album << endl;
